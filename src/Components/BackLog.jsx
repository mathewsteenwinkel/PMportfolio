...
<div className="text-sm text-slate-600">Items Delivered</div>
            ...
            <div className="border-t border-slate-200">
              {epic.items.map((item, index) => (
                <div
                  key={item.id}
                  className={`p-6 ${
                    index !== epic.items.length - 1 ? 'border-b border-slate-100' : ''
                  }`}
                >
    ...
    <div className="text-center mt-8 text-sm text-slate-500">
      <p>mathewsteenwinkel@gmail.com • 780-909-6495 • Canmore, AB</p>
      <p className="mt-1">linkedin.com/in/mathew-steenwinkel</p>
    </div>
  </div>
</div>
);
